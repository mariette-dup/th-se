...
Ce code permet de tester deux méthodes de réduction du nombre de variables descriptives et deux méthodes de classification afin de trouver la meilleure combinaison des deux méthodes pour discriminer des patients sains des patients pathologiques.  


# Base de données 

Pour que ce code fonctionne il faut que les premières lignes de la base de données soit des patients controles et les lignes suivantes des patients pathologiques.  

```{r}
#importation de la base de données
data_davd <- read.csv("U:/th-se/DAVD_nom_new.csv" , sep=";",header = TRUE)
#data_davd <- read.csv("DAVD_nom.csv" , sep=";",header = TRUE)
```


```{r}
#récupération des noms de variables créés par Nolwenn TAN : 1680 variables descriptives
Name_desc <- read_excel("U:/th-se/Name_desc.xlsx")
#Name_desc <- read_excel("Name_desc.xlsx")
```

Pour mettre en place l'ACP par la suite il faut enlever les valeurs manquantes (NA). Le code permet de visualiser quels individus ont des NA (dans la liste indNA) puis les enlève. vERIFIER AVANT SI NA SINON LE CODE SUIVANT VA VIDER LA BASE DE DONNEES. 

```{r}
#indNA <- which(is.na(data_davd),arr.ind=TRUE)[,1]
#data_davd <- data_davd[-indNA,]
```

Cette partie du code permet de construire la variable réponse, il faut alors connaitre le nombre de patients sains construire le vecteur correctement. 

```{r}
nb_total = nrow(data_davd)
nb_sains = 27
nb_pathos = nb_total-nb_sains
```
...
Ajout des noms de variables comme noms de colonnes dans la base de données. 

```{r}
for(i in 1:ncol(data_davd)){
